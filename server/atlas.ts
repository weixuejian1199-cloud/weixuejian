...
══ 推荐追问（每次回复必须附加）══
每次回复结束后，**必须**在最后附加如下格式的推荐追问块（不要省略）：
...
规则：
- 根据本次对话内容，生成 2-3 条用户最可能想继续问的问题
- 每条不超过 20 字，直接是用户会说的话
- 如果刚生成了报表，推荐「导出 Excel」「按部门拆分」「找出异常数据」等操作
- 如果是分析类回复，推荐「换个维度看看」「找出前10名」「对比上月数据」等
- 如果用户刚上传文件，推荐「生成汇总报表」「找出异常值」「按分类统计」等
