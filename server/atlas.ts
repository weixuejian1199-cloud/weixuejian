...
「用户」「人员」「员工」「记录」「条」 = 指行（人员）
「所有用户」「全部」「全量」「所有人」 = 必须展示全量数据，不得截断；如超过20行，在表格标题后注明「共[N]条，展示前20条，完整数据请导出 Excel」（[N]替换为实际总行数）
...
` : '';
      ...
      const categoryStatsContext = (singleFileCategoryStats && Object.keys(singleFileCategoryStats).length > 0) ? (() => {
        ...
        for (const [fieldName, entries] of Object.entries(singleFileCategoryStats)) {
          ...
          const hasSum = top10.some(e => e.sum !== undefined);
          if (hasSum) {
            // 有金额统计：展示 count + sum + avg
            const entryLines = top10.map(e =>
              `  ${e.label}: 订单数${e.count}单${e.sum !== undefined ? `，金额${e.sum.toFixed(2)}，均单价${e.avg?.toFixed(2)}` : ''}`
            );
            lines.push(`${fieldName}分布（按订单数降序，全量数据 ${dfInfo.row_count.toLocaleString()} 行）：`);
            lines.push(...entryLines);
          } else {
            // 只有 count
            const entryLines = top10.map(e => `  ${e.label}: ${e.count}单`);
            lines.push(`${fieldName}分布（按订单数降序，全量数据 ${dfInfo.row_count.toLocaleString()} 行）：`);
            lines.push(...entryLines);
          }
        }
...
⚠️ 以下统计来自全量数据，回答省份/支付方式/城市/状态等分类统计时，必须直接引用以下数据。
  ...
  const sections = perFileProfiles.map((pfp, idx) => {
    ...
    const pfpCategoryStatsContext = (pfpCategoryStats && Object.keys(pfpCategoryStats).length > 0) ? (() => {
      ...
      for (const [fieldName, entries] of Object.entries(pfpCategoryStats)) {
        ...
        const hasSum = top10.some(e => e.sum !== undefined);
        if (hasSum) {
          catLines.push(`${fieldName}分布（全量 ${pfp.rowCount.toLocaleString()} 行）：`);
          top10.forEach(e => catLines.push(`  ${e.label}: ${e.count}单${e.sum !== undefined ? `，金额${e.sum.toFixed(2)}` : ''}`));
        } else {
          catLines.push(`${fieldName}分布（全量 ${pfp.rowCount.toLocaleString()} 行）：`);
          top10.forEach(e => catLines.push(`  ${e.label}: ${e.count}单`));
        }
      }
      if (catLines.length === 0) return '';
      return `\n══ 分类字段全量统计（全量数据，非样本）══\n⚠️ 以下统计来自全量数据，禁止对 sample_rows 做 GROUP BY / COUNT 得出分类统计。\n${catLines.join('\n')}\n══ 分类统计结束 ══`;
    })() : '';
...
══ 跨文件商品排名结束 ══` : '';
  ...
  const crossFileCategorySection = (() => {
    ...
    for (const fieldName of Array.from(allCategoryFields)) {
      ...
      const totalRows = perFileProfiles.reduce((acc, pfp) => acc + pfp.rowCount, 0);
      const sorted = Array.from(unionCountMap.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10);
      crossLines.push(`${fieldName}分布（跨文件合并，全量 ${totalRows.toLocaleString()} 行）：`);
