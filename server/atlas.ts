      ...
      setImmediate(async () => {
        try {
          ...
          const finalResult = {
            ...
            file_url: fileUrl,
            status: "ready",
            stored_row_count: dfInfo.row_count, // 修复项 B：返回实际存储行数，与前端 totalRowCount 校验
            df_info: {
...
` : '';
      ...
      const categoryStatsContext = (singleFileCategoryStats && Object.keys(singleFileCategoryStats).length > 0) ? (() => {
        const lines: string[] = [];
        for (const [catKey, entries] of Object.entries(singleFileCategoryStats)) {
          if (!entries || entries.length === 0) continue;
          // 修复项 C：从第一个 entry 获取字段元数据
          const firstEntry = entries[0] as any;
          const fieldName = firstEntry?.fieldName || catKey; // 原始字段名（如「收货省份」）
          const categoryKey = firstEntry?.categoryKey || catKey; // 标准 key（如 "province"）
          ...
          if (hasSum) {
            // 有金额统计：展示 count + sum + avg
            const entryLines = fullEntries.map(e =>
              `  ${e.label}: 订单数${e.count}单${e.sum !== undefined ? `，金额${e.sum.toFixed(2)}，均单价${e.avg?.toFixed(2)}` : ''}`
            );
            lines.push(`${fieldName}分布（category_key=${categoryKey}，共${fullEntries.length}个分类，全量数据 ${dfInfo.row_count.toLocaleString()} 行，以下为全部分类，按订单数降序）：`);
...
[INFO] 修复项C：当你输出 atlas-table 时，必须在 JSON 中包含 category_key 字段（如 "category_key": "province"）。这是系统导出的强绑定依据，不得省略。
${lines.join('\n')}
══ 分类统计结束 ══
`;
      })() : '';
...
- 只返回JSON，不要其他文字`;
      ...
      setImmediate(async () => {
        try {
          ...
          const finalResult = {
            ...
            stored_row_count: dfInfo.row_count, // 修复项 B：返回实际存储行数
  ...
  app.post("/api/atlas/upload-parsed", optionalAuth, async (req: Request, res: Response) => {
    try {
      ...
      const parsed = req.body as {
        ...
        // Category stats: full-dataset GROUP BY stats for ALL categorical fields
        categoryGroupedTop20?: Record<string, Array<{ label: string; count: number; sum?: number; avg?: number }>>;
        // 修复项 B：全量数据行（仅当 totalRowCount ≤ 50000 时前端内联传输）
        // preview 只用于预览，不作为业务真源
      ...
      setImmediate(async () => {
        try {
          // 修复项 B：优先使用 allRows（全量数据）作为业务真源
          // 如果前端没有传 allRows（> 50000 行的大文件，强制走 chunk 路径），回退到 preview（应尽量避免）
          const sourceData = parsed.allRows?.length ? parsed.allRows : (parsed.preview || []);
          const dataSource = parsed.allRows?.length ? 'allRows(全量)' : 'preview(降级)';
          console.info(`[ATLAS upload-parsed] ${parsed.filename}: 使用 ${dataSource} 存储，共 ${sourceData.length} 行`);
          ...
          const requiredFields = requiredByScenario[scenarioHint.type] || [];
          const { normalizedData, fieldMapping } = normalizeFieldNames(sourceData, requiredFields);
          const storedRowCount = normalizedData.length;

          await storeSessionData(sessionId, normalizedData);
          await updateSession(sessionId, {
            rowCount: storedRowCount, // 使用实际存储行数更新
            colCount: dfInfo.col_count,
            // 修复项 B：将 storedRowCount 存入 dfInfo 扩展字段，与前端 totalRowCount 校验
            dfInfo: { ...dfInfo as any, row_count: storedRowCount, stored_row_count: storedRowCount },
          }).catch(() => {});
          ...
          const finalResult = {
            ...
            file_url: "",
            status: "ready",
            df_info: { ...dfInfo, row_count: storedRowCount }, // 使用实际存储行数
            ai_analysis: aiAnalysis,
            suggested_actions: suggestedActions,
            quality_issues: qualityIssues,
            // 修复项 B：返回实际存储行数，前端校验用
            stored_row_count: storedRowCount,
            data_source: dataSource, // 'allRows(全量)' | 'preview(降级)'
          };
          await storeUploadResult(sessionId, finalResult);
          await updateSession(sessionId, { status: "ready" });
          console.log(`[Atlas] upload-parsed complete: ${sessionId}, storedRows=${storedRowCount} (${dataSource})`);
        } catch (bgErr: any) {
  ...
  // ── POST /api/atlas/upload-rows ───────────────────────────────────────────────────────────────────────────────────
  // 大文件全量数据分块上传：前端把 allRows 分批 POST 到此端点，服务端追加写入 S3
  // 每批最多 5000 行，最终调用方在所有批次完成后发送 finalize=true
  app.post("/api/atlas/upload-rows", optionalAuth, async (req: Request, res: Response) => {
    try {
      const { sessionId, rows, batchIndex, totalBatches, finalize } = req.body as {
        sessionId: string;
        rows?: Record<string, unknown>[];
        batchIndex: number;
        totalBatches: number;
      ...
      const BATCH_KEY = (sid: string, idx: number) => `atlas-data/${sid}/batch_${String(idx).padStart(4, '0')}.json`;

      if (!finalize) {
        // 存储当前批次数据
        if (!rows || !Array.isArray(rows)) { res.status(400).json({ error: "rows required" }); return; }
        await storagePut(BATCH_KEY(sessionId, batchIndex), JSON.stringify(rows), "application/json");
        res.json({ ok: true, batchIndex, received: rows.length });
      } else {
        // finalize=true：合并所有批次，覆盖主数据文件
        const allMergedRows: Record<string, unknown>[] = [];
        for (let i = 0; i < totalBatches; i++) {
          try {
            const { url } = await storageGet(BATCH_KEY(sessionId, i));
            const batchRes = await fetch(url);
            if (batchRes.ok) {
              const batch = await batchRes.json() as Record<string, unknown>[];
              allMergedRows.push(...batch);
            }
          } catch (e) {
            console.warn(`[upload-rows] batch ${i} not found for session ${sessionId}`);
          }
        }
        if (allMergedRows.length === 0) {
          res.status(400).json({ error: "No rows received in any batch" });
          return;
        }
        // 规范化字段名（与 upload-parsed 保持一致）
        const { normalizedData } = normalizeFieldNames(allMergedRows, []);
        // 覆盖主数据 S3 key（覆盖 preview 降级数据）
        await storeSessionData(sessionId, normalizedData);
        // 更新 session 的 storedRowCount
        const currentDfInfo = (session.dfInfo as any) || {};
        await updateSession(sessionId, {
          rowCount: normalizedData.length,
          dfInfo: {
            ...currentDfInfo,
            stored_row_count: normalizedData.length,
            row_count: currentDfInfo.row_count ?? normalizedData.length,
          },
        }).catch(() => {});
        // 更新 S3 result 文件中的 stored_row_count
        try {
          const resultUrl = (await storageGet(RESULT_KEY(sessionId))).url;
          const resultRes = await fetch(resultUrl);
          if (resultRes.ok) {
            const result = await resultRes.json() as Record<string, unknown>;
            result.stored_row_count = normalizedData.length;
            result.data_source = 'allRows(全量-分批)';
            await storagePut(RESULT_KEY(sessionId), JSON.stringify(result), "application/json");
          }
        } catch (e) {
          console.warn(`[upload-rows] result update failed for ${sessionId}:`, e);
        }
        console.log(`[upload-rows] finalized: ${sessionId}, totalRows=${normalizedData.length}`);
        res.json({ ok: true, totalRows: normalizedData.length, data_source: 'allRows(全量-分批)' });
      }
    } catch (err: any) {
      console.error("[upload-rows] error:", err);
      res.status(500).json({ error: err.message || "Upload rows failed" });
    }
  });
