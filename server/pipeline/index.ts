...
// ── 从已解析数据运行管道（跳过 Ingestion 层）──────────────────────

/**
 * V3.0: 从前端已解析的 JSON 数据运行管道。
 * 跳过 Ingestion 层（Step 1-3），直接从 Step 4（平台识别）开始。
 * 用于 upload-parsed 端点（前端用 SheetJS 解析后发送 JSON）。
 */
export interface ParsedDataPipelineInput {
  /** 前端解析的全量行数据 */
...
export async function runPipelineFromParsedData(
  ...
  try {
    if (!input.rows || input.rows.length === 0) {
      ctx.aborted = true;
      ctx.abortReason = "没有数据行";
      return buildErrorOutput(ctx);
    }

    // 提取表头
    const headers = Object.keys(input.rows[0]).filter(
    ...
    // ── Step 4: 平台识别 ──────────────────────────────────────
    const { step4PlatformDetect, step5FieldMapping } = await import("./ingestion");
    const step4 = step4PlatformDetect(ctx, headers);

    // ── Step 5: 字段映射 ──────────────────────────────────────
    const step5 = step5FieldMapping(ctx, headers);
    if (ctx.aborted) {
      return buildErrorOutput(ctx);
    }

    // 将行数据转为 Record<string, string>[] 格式（Pipeline 要求字符串值）
    const rawRows: Record<string, string>[] = input.rows.map(row => {
    ...
    // ── Layer 2: Governance ──────────────────────────────────────
    const governance = runGovernance(ctx, rawRows, step5.mappedFields);
    ...
    // ── Layer 3: Computation ──────────────────────────────────────
    const allFields = new Set<string>();
    for (const [, stdName] of Object.entries(step5.mappedFields)) {
      allFields.add(stdName);
    }

    const sourceFiles: SourceFileInfo[] = [{
      fileName: input.fileName,
      s3Key: "", // upload-parsed 没有 S3 key
      totalRows: input.rows.length + 1,
      dataRows: input.rows.length,
