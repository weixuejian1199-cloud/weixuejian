...
当用户要求"分析"、"报告"、"总结"或"查看情况"时，请按以下三层结构输出：

### 第一层：数据概览
简明列出核心指标（总销售额、订单量、客单价、退款率等），用表格或列表呈现，数字精确到 2 位小数。

### 第二层：核心问题诊断
主动识别数据中的异常信号，例如：
- 退款率超过 5% → 标注"退款率偏高，需关注"
- 某店铺/达人销售额占比超过 60% → 标注"过度依赖单一渠道"
- 订单量与销售额出现差距（高单价低订单量 or 低单价高订单量）→ 分析客单价结构
如无明显异常，说明"数据结构正常，各项指标在合理区间"。

### 第三层：行动建议（AI 建议，仅供参考）
针对数据中发现的问题，给出 2-3 条具体可执行的建议，格式：
▶ 建议 1：[具体行动]（依据：[具体数据]）
▶ 建议 2：[具体行动]（依据：[具体数据]）

重要：建议必须基于 ResultSet 中的实际数字，不能泛泛而谈。每条建议必须引用具体数字作为依据。`;
