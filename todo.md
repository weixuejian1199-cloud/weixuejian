...
### 聊天界面升级
- [ ] 飞书风格输入框：底部工具栏（Aa/表情/At/附件/全屏/发送）
- [ ] 文件上传区：拖拽或点击附件图标上传，支持图片/文档/Excel
- [ ] 文件预览：上传后在输入框上方显示文件缩略图/图标+文件名+删除按钮
- [ ] 消息气泡优化：发送中状态（灰色+loading圈）、已发送状态、失败状态（红色!+重试）
- [ ] 错误提示：发送失败时气泡变红，点击可重试
- [ ] loading 状态：AI 回复前显示打字指示器（三点动画）

### 流式回复（打字机效果）
- [ ] 打字机效果：AI 回复到达后前端逐字显示（每字间隔约 30ms）
- [ ] 中断提示：打字机动画中途如果新消息到达，立即切换到新消息
- [ ] 完成落库：打字机动画完成后消息状态更新为完整
- [ ] 降级模式：如果流式失败，回退到一次性显示完整回复

## Phase 2A 补充需求

- [ ] 消息撤回：发送后 2 分钟内右键/长按气泡可撤回，撤回后显示「消息已撤回」
- [ ] 全屏编辑：输入框工具栏全屏按钮弹出全屏编辑器（支持 Markdown 预览），确认后发送

## Phase 2B 补充：WebSocket 掉线自动重连

- [ ] 服务端心跳间隔从 30s 改为 25s（确保在 Cloudflare 100s 超时前刷新）
- [ ] 客户端检测到 WebSocket 断连后 3s 自动重连（指数退避，最多 30s）
- [ ] 重连时顶部显示「连接已断开，正在重连...」黄色提示条
- [ ] 重连成功后提示条消失，恢复正常状态
- [ ] 重连后自动重新加载当前会话消息（防止断线期间消息丢失）

## Phase 2A 补充：消息复制

- [ ] 消息气泡 hover 时右下角显示「复制」按钮，点击复制该条消息内容
- [ ] 对话框底部「复制全部」按钮：一键复制当前对话所有消息（格式：发送方: 内容\n）
- [ ] 复制成功后显示 Toast 提示「已复制」

## Phase 2A 聊天体验升级

- [x] WebSocket 自动重连（指数退避 3s→6s→12s→30s，顶部黄色提示条）
- [x] 飞书风格输入框组件 FeishuChatInput（工具栏：Aa/表情/@/附件/全屏/发送）
- [x] 全屏编辑器 FullscreenEditor（飞书风格：顶部格式工具栏 + 大面积编辑区 + 底部工具栏）
- [x] 消息撤回（2分钟内右键菜单撤回，显示「消息已撤回」）
- [x] 打字机效果 hook（useTypewriter）
- [x] 消息复制（自己消息左下角，对方消息右下角，hover 显示，复制成功 toast）
- [x] MainWorkspace 输入框升级为 FeishuChatInput

## Phase 2B 稳定性补强

- [x] WebSocket 断线重连机制（已在 Phase 2A 完成）
- [x] 发送超时检测（8s）+ 重试机制
- [x] 消息去重（msgId 去重）
- [x] 未读提醒（会话列表未读数角标）
...
- [x] 意图识别（基于规则，毫秒级，无需 API 调用）
- [x] 数据分析 Agent（专注 Excel/CSV 分析、报表生成、图表输出）
- [x] HR Agent（工资条、考勤、个税计算）
- [x] 质量监控 Agent（小虾米/OpenClaw 监控 Qwen 回复质量）
- [x] 通用助手 Agent（问答、使用引导、业务知识）
- [x] 前端显示当前 Agent 类型标签
- [x] AI 消息下方 suggestedActions 快捷操作按钮

## Phase 4 数据报告核心功能

- [x] AtlasChartRenderer 图表渲染组件（柱状图/折线图/饼图/面积图）
- [x] parseAtlasChartBlocks 解析 atlas-chart 代码块
- [x] MainWorkspace 消息渲染集成图表渲染
- [x] buildExpertSystemPrompt 升级：支持 atlas-chart 图表输出格式
- [x] agentRouter data_analysis Agent 升级：支持 atlas-chart 图表输出

## Phase 2A 补充需求

- [x] 消息撤回：发送后 2 分钟内右键/长按气泡可撤回，撤回后显示「消息已撤回」
- [x] 全屏编辑：输入框工具栏全屏按钮弹出全屏编辑器（飞书样式），确认后发送
- [x] 消息复制：自己消息左下角，对方消息右下角，hover 显示复制按钮，复制成功 toast
...
## 游客模式（2026-03-10）
- [ ] 移除强制登录限制：未登录用户可直接使用所有基础功能
- [ ] 仅在调用 protectedProcedure 时才弹出登录提示（而非页面加载时）
- [ ] 导航栏未登录状态显示「登录」按钮，而非隐藏用户信息区

## Bug 修复（2026-03-10）
- [x] 修复登录弹窗登录成功后不自动关闭（欢迎 toast 出现但弹窗卡住）
- [x] 实现游客模式：未登录用户可直接使用，仅在需要时才提示登录

## 紧急 Bug 修复（2026-03-10）
- [ ] 修复聊天页面文件上传失败
- [ ] 修复聊天页面消息发送失败
- [ ] 恢复原有思考/分析流程（不改变 UI）

## 功能恢复与布局调整（2026-03-10）
- [ ] 恢复之前版本中已实现的「分享好礼」功能（被占位按钮替换了）
- [ ] 文件模块重构为 20/20/60 三栏布局
- [ ] AI工具模块重构为 20/20/60 三栏布局
- [ ] 自动化模块重构为 20/20/60 三栏布局
- [ ] 知识库模块重构为 20/20/60 三栏布局
- [ ] 修复聊天页面文件上传失败
- [ ] 修复聊天页面消息发送失败

## V16.0 视觉重构（设计稿还原）

- [x] 整体色调切换为浅蓝色调（参考 IMG_2453 + IMG_2454）
- [x] 顶部通栏：「ATLAS | 一个模块，一种心智。」+ 三个图标（Menu/Info/Heart）
- [x] 左侧导航：字体调大、AI引擎分组、底部分享卡片、浅蓝色调
- [x] 对话区空状态：蓝紫渐变气泡（靠左，正常大小，右侧40%留白），三个快捷建议卡片
- [x] 用户消息气泡：蓝紫渐变 + 光感（linear-gradient + inset shadow）
- [x] 右侧面板空状态：3D数据球（CSS球体 + 网格线 + 节点连线 + 浮动图表卡片动效）
- [x] 输入框：玻璃拟态效果（backdrop-filter + 半透明背景）
- [x] 整体背景：浅蓝灰渐变（#EEF2F7）

## V16.1 导航微调

- [ ] 移除左侧导航「AI 引擎」分组标题及下面三个条目（门店导出、3月即存集、重庆劝拨江）

## V16.2 紧急修复（功能断裂）

- [ ] 修复登录界面无法登录（LoginModal 按钮无响应）
- [ ] 修复对话功能失效（ChatWorkspace 无法发送消息 / AI 无响应）
- [ ] 修复文件上传失效（上传按钮无响应或报错）
- [ ] 知识库页面游客可访问（不强制登录）
- [ ] 移除左侧导航「AI 引擎」分组及三个条目

## V16.2 布局修复 + 导航清理

- [x] 移除左侧导航「AI 引擎」分组及三个条目（门店导出/3月即存集/重庆劝拨江）
- [x] FilesModule 布局改为 20%（文件列表）+ 60%（预览）+ 20%（信息面板）
- [x] AIToolsModule 布局改为 20%（工具列表）+ 80%（内容区）
- [x] AutomationModule 布局改为 20%（自动化列表）+ 80%（内容区）
- [x] KnowledgeModule 布局改为 20%（知识库列表）+ 80%（内容区）
- [x] 确认后端 API 全部正常（登录/上传/对话），白屏为平台限流所致

## V16.3 导航重复修复

- [x] 移除左侧导航顶部 NavLogo 组件（logo 重复三次）
- [x] 用户头像/登录按钮移到 TopBar 右上角
- [x] 恢复侧栏折叠/展开按钮（TopBar 左侧 PanelLeft 图标）

## V16.4 对话框顶部工具栏（Manus 风格）

- [ ] 对话框顶部加模型选择器（Qwen3-Max / Kimi-K2 下拉，有对话时显示）
- [ ] 对话框顶部右侧加分享按钮（📤）
- [ ] 对话框顶部右侧加「查看此任务所有文件」按钮（📎）
- [ ] 对话框顶部右侧加更多选项按钮（⋯，含重命名/删除任务等）

## V16.5 移除机器人列表

- [ ] 移除「对话」分组下的 ATLAS 和 OpenClaw 子项（机器人列表）

## UI 反馈修复（2026-03-10 大魏反馈）

### 🔴 高优先级
- [x] Bug 1：文件界面「用这个文件打开新对话」点击无响应，文件数据无法传入对话窗

### 🟡 中优先级
- [x] Bug 2：三栏布局比例不合理（中间太窄），调整为左20%/中35%/右45%
- [x] Bug 3：左侧导航布局优化——知识库移到底部（仅图标）、用户头像移到右上角
- [x] Bug 4：侧边栏开关按钮位置不对，参考 Manus 风格调整

### 🟢 低优先级
- [x] Bug 5：文件预览区数据单一且不专业，增加可视化图表和统计卡片
- [x] Bug 6：最近对话字体太小，与「对话/文件/工具」字体差距过大，稍微放大
- [x] Bug 7：「与好友分享」组件高度偏高，降低高度使其更紧凑
- [x] Bug 8：品牌标语「ATLAS 一个模块，一种心智」视觉权重不足，增加字体/颜色强调

## V16.6 紧急 Bug：登录后无法发消息/上传文件（2026-03-10 大魏反馈）

- [x] Bug：登录状态下无法发消息/上传文件，新浏览器登录可以，旧浏览器不行
  - 现象：旧浏览器有残留 cookie/JWT，但 token 已失效，请求被后端拒绝，前端无提示/无重定向
  - 方向：检查 JWT 过期处理、cookie 清理逻辑、前端 401 错误处理

## V16.7 紧急修复（2026-03-10 大魏反馈）

- [x] 紧急 Bug A：账户无法登录（所有浏览器）——context.ts 使用 req.cookies（需要 cookie-parser）导致全局崩溃，已改回 parseCookieHeader
- [x] 紧急 Bug B：所有浏览器无法发消息——与 Bug A 同源，已随 Bug A 一起修复
- [x] UI Bug C：工具/自动化/知识库等模块三栏比例未调整，已同步修复
- [x] UI Bug C：文件/工具/自动化/知识库四个模块三栏比例从 20%/20%/60% 改为 20%/35%/45%
