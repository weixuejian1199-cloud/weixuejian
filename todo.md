...
- [x] C1: index.css 全局主题换肤（Gemini 设计系统 + 所有 --atlas-* 变量映射到 Gemini 色值）
- [x] C2: Sidebar 极简重写（新建对话 + 历史列表 + 分享 + 设置，按 UI 规格文档）
- [x] C3: TopBar 简化（无分割线，与页面背景 #f0f4f9 融为一体）
- [x] C4: MainWorkspace 欢迎页换肤（Gemini 风格 ✦ Hi + 4 个快捷标签）
- [x] C5: 输入框样式更新（圆角 24px、max-width 680px、品牌色聚焦）
- [x] C6: 手机端适配 — 响应式 isMobile 状态 + 侧边栏自动折叠 + 遮罩层
- [x] C7: 消息气泡样式更新 — 用户气泡 #e8eaed、AI 无背景、操作按钮品牌色
- [ ] C8: 单元测试 — UI 组件测试

## C 阶段 UI 重大修复（基于用户反馈 2026-03-13）

- [ ] C-FIX-1: 首页布局改为 Gemini 风格 — 对话框垂直居中偏上(40%)，不在底部
- [ ] C-FIX-2: 快捷标签移到对话框下方（不在底部）
- [ ] C-FIX-3: 侧栏宽度改为 200px，与主内容区有颜色层次感区别
- [ ] C-FIX-4: 侧栏
搜索改为搜索图标（☰ 🔍 排列），不要搜索框
- [ ] C-FIX-5: 新建对话用 ✏️ 在搜索图标下方单独一行
- [ ] C-FIX-6: 实现文件面板（📎 回形针 + 右侧滑出 320px 面板）
- [ ] C-FIX-7: TopBar 右侧添加 📎 文件图标 + ⋮ 更多菜单 + 用户头像
- [ ] C-FIX-8: 对话流中对话框固定底部（区别于首页居中）
...
- [ ] D-UI-2: 首页布局重写（对话框居中40%、快捷标签在下方）
- [ ] D-UI-3: TopBar 重写 + 文件面板实现
- [ ] D-UI-4: 全局样式调整（侧栏与主区域颜色层次）


## E 阶段：UI 全面修正（2026-03-13 用户反馈第二轮）

- [x] E-1: 侧栏完整功能模块入口（新建对话/模板库/HR中心/数据中枢/文件/AI工具/自动化/知识库），未上线标注"即将上线"
- [x] E-2: 侧栏搜索改为图标（☰ + 🔍 排列），点击展开内联搜索
- [x] E-3: TopBar 不要大搜索框
- [x] E-4: 📎文件回形针可点击使用（不是待开放），打开文件面板
- [x] E-5: 文件面板右侧滑出320px，显示已上传文件列表
- [x] E-6: 首页 Gemini 风格对话框居中40%
- [x] E-7: 快捷标签/模板在对话框下方
- [x] E-8: 侧栏与主区域颜色层次感
- [x] E-9: 分享好礼功能可用（复制分享链接）
- [x] E-10: 全局样式调整（侧栏与主区域颜色层次感 CSS 变量）
