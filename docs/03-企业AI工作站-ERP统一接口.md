...
> 版本：v3.3 · 2026-04-02 · 本文档定义 ERP/商城/RPA 适配层的统一接口契约，所有适配器必须严格遵守
> v3.3 变更：新增 RPAAdapter（Playwright RPA）支持财务/运营数据采集（ADR-031）；补充聚水潭API限制说明（仅订单数据，无财务数据）
...
适配层（AdapterFactory 按 type 分发）
    ├── MallAdapter（ztdy-open 商城）  ← Phase 1 订单数据源（ADR-024）
    ├── RPAAdapter（Playwright RPA）   ← Phase 1 财务/运营数据源（ADR-031，只读不写）
    ├── JSTAdapter（聚水潭）           ← 注意：仅订单数据，无财务数据
    ├── WDTAdapter（旺店通）
    ├── GYYAdapter（管易云）
    └── CSVAdapter（CSV/Excel导入 + 用户上传分析）
    ↕
各家 ERP/商城 API + RPA 浏览器采集
```

### MallAdapter（Phase 1 订单数据源）

> ztdy-open 是现有运营中的商城系统（极速订货），Phase 1 通过 API 读取真实数据验证 AI 能力。
> Phase 2 自建商城上线后，MallAdapter 退役，切换为同库直查。适配器模式让过渡零痛感。

### RPAAdapter（Phase 1 财务/运营数据源，ADR-031）

> **驱动原因**：聚水潭API只能获取订单数据，**拿不到财务数据**（平台结算单、应收款、佣金明细、推广费用等），小红书/美团API对小商家几乎不开放。
> **技术选型**：Playwright（Node.js），与项目技术栈一致。
> **核心原则**：绝对只读不写。RPA只用于数据采集，不做任何写操作（不改价/不发货/不回评/不投放）。
> **授权方式**：管理员在工作站绑定各平台账号（cookie加密存储），各岗位按权限透明使用数据。
> **替换策略**：DataSourceAdapter 统一接口，平台API开放后 RPASource 无缝替换为 APISource，业务层零修改。
> 详见 docs/14-RPA数据桥梁方案.md

| 采集数据 | 来源平台 | 频率 | 用途 |
|---------|---------|------|------|
| 平台结算单 | 抖店/天猫/美团 | 每日 | 财务对账、生成凭证 |
| 佣金/服务费明细 | 抖店/天猫/美团 | 每月 | 成本核算、费用归集 |
| 推广费用明细 | 千川/直通车/引力魔方 | 每日 | 推广费归集、ROI核算 |
| 退款/扣款明细 | 抖店/天猫 | 每日 | 退货退款凭证 |
| 每日经营概况 | 抖店罗盘/生意参谋 | 每日 | 运营概览KPI |
| 投放计划效果 | 千川/万相台 | 每日 | 投放管理看板 |
